...
/usr/sbin/iptables -N $CHAIN_NAME &> /dev/null
...
if [ $MSS -gt 100 ]; then 
    /usr/sbin/iptables -A FORWARD -p tcp -i $SERVER_INTERFACE --tcp-flags SYN,RST SYN -j TCPMSS --set-mss $MSS &> /dev/null
    /usr/sbin/iptables -A FORWARD -p tcp -o $SERVER_INTERFACE --tcp-flags SYN,RST SYN -j TCPMSS --set-mss $MSS &> /dev/null
fi

/usr/sbin/iptables -A $CHAIN_NAME -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT

/usr/sbin/iptables -A FORWARD -i $SERVER_INTERFACE -j $CHAIN_NAME
/usr/sbin/iptables -A FORWARD -i $SERVER_INTERFACE -j DROP
/usr/sbin/iptables -A FORWARD -o $SERVER_INTERFACE -j ACCEPT
...
/usr/sbin/ip6tables -N $CHAIN_NAME &> /dev/null
/usr/sbin/ip6tables -F $CHAIN_NAME 

/usr/sbin/ip6tables -A $CHAIN_NAME -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT

/usr/sbin/ip6tables -A FORWARD -i $SERVER_INTERFACE -j $CHAIN_NAME
/usr/sbin/ip6tables -A FORWARD -i $SERVER_INTERFACE -j DROP
/usr/sbin/ip6tables -A FORWARD -o $SERVER_INTERFACE -j ACCEPT
