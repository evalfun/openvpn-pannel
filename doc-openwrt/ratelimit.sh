#!/bin/bash
# 达量限速运行时更新脚本（OpenWrt / ImmortalWrt 版本，资源名 ratelimit.sh）
...
#
# OpenWrt 说明：
#   - 无需 sudo，直接调用绝对路径的 tc；需安装 tc 及内核模块
#     sch_htb / cls_u32 / act_police / sch_ingress。
#   - 与 client_online.sh / client_offline.sh 共用同一把 flock（__WORKING_DIR__acl.lock）。
...
(
if ! flock 9; then
    log_message "WARNING" "服务器 $SERVER_ID 获取限速锁失败，跳过达量限速更新"
    exit 0
fi
