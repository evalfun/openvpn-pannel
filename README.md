...
- 内置证书管理：用 Go 生成/签发 CA、服务器与客户端证书，**不依赖 openssl**，支持导入与引用
...
- **一级页面（CA 列表）**：生成新 CA、导入 CA；每个 CA 可查看内容、进入管理、删除。
- **二级页面（某 CA 下）**：用该 CA 签发**服务器证书**或**客户端证书**，也可导入证书。
  - 导入的证书必须由该 CA 签发，否则拒绝；**可只导入证书、不带私钥**。
  - 若 CA 没有私钥，则无法用它签发新证书（仍可查看/导入其签发的证书）。
- 密钥类型支持 **RSA**（2048/3072/4096）与 **ECDSA**（P-256/P-384/P-521），创建时选择。

服务器编辑处，CA 证书 / 服务器证书 / 服务器私钥既可**直接粘贴 PEM**，也可从证书管理**选择引用**；
选择引用时数据库存储 `cert-stor:<证书ID>/cert` 或 `cert-stor:<证书ID>/key`，写入实例目录前由面板解析为实际内容。

- **DH 参数**：一键生成，使用 RFC 3526 标准 MODP 组（2048/3072/4096），等价于标准 `dhparam`，无需漫长计算；已有值时会提示覆盖。
- **TLS-Auth**：可用 `openvpn --genkey` 一键生成 `ta.key`。
