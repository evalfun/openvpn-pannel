...
- **IPv6 支持**：ACL、带宽限速、流量统计与客户端自助页面均支持 IPv6（iptables 与 nftables 两套脚本
  均已支持，含 OpenWrt）；当客户端经 IPv6 端点接入时，脚本会以 `untrusted_ip6` / `trusted_ip6`
  取得客户端真实地址并记录到日志与事件中（详见面板内帮助）；是否开启服务器 IPv6 建议按业务实际情况决定
...
- **脚本可灵活配置**：认证、上下线、ACL、限速等逻辑全部由脚本资源驱动，可在面板内在线查看与替换；
  程序内置 `iptables` 与 `nftables`、`标准Linux` 与 `OpenWrt/ImmortalWrt` 共4套脚本，在管理面板里可以一键切换。
